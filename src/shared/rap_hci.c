...
// ======================
typedef void (*cs_callback_t)(uint16_t index, uint16_t length,
...
void cs_state_machine_init(struct cs_state_machine_t *sm, struct bt_rap *rap,
		struct bt_hci *hci, struct rap_utils *utils)
{
	if (!sm) return;
...
void cs_set_state(struct cs_state_machine_t *sm, enum cs_state_t new_state)
{
	...
	if (sm->current_state > CS_UNSPECIFIED || new_state > CS_UNSPECIFIED) {
		if (sm->utils)
			DBG(sm->utils, "[ERROR] Invalid state transition attempted\n");
		return;
	}

	if (sm->utils) {
		DBG(sm->utils, "[STATE] Transition: %s → %s\n",
		...
		void *user_data)
{
	...
	if (!sm || !data ||
		size < sizeof(struct hci_rp_le_cs_set_default_settings)) {
		return;
	}

	if (sm->utils)
		DBG(sm->utils,"[EVENT] CS default Setting Complete (size=0x%02X)\n",
			size);
	...
	if(CS_INIT != cs_get_current_state(sm)) {
		if (sm->utils)
			DBG(sm->utils,"Event received in Wrong State!! Expected : CS_INIT");
		return;
	}
	 ...
	 if (rp->status == 0) {
		...
		//Reflector role
		if (sm->utils)
			DBG(sm->utils,"Waiting for CS Config Completed event...\n");
		//ToDO : Initiator role - Send CS Config complete cmd
	} else {
		// Error - transition to stopped
		if (sm->utils)
			DBG(sm->utils,"[ERROR] CS Set default setting failed with\
				status 0x%02X\n", rp->status);
		...
		struct hci_evt_le_cs_read_rmt_supp_cap_complete *ev)
{
	...
	if(!sm || !sm->hci) {
		DBG(sm->utils,"[ERR] Set Def Settings: sm or hci is null");
		return;
	}
	status =  bt_hci_send(sm->hci, HCI_OP_LE_CS_SET_DEFAULT_SETTINGS,
				&cp, sizeof(cp), rap_def_settings_done_cb, sm, NULL);
	DBG(sm->utils,"sending set default settings case, status : %d", status);
	if (!status)
		DBG(sm->utils,"Failed to send default settings cmd");
		...
		void *user_data)
{
	...
	if (!sm || !data ||
		size < sizeof(struct hci_evt_le_cs_read_rmt_supp_cap_complete)) {
		return;
	}
	...
	if (sm->utils)
		DBG(sm->utils,"[EVENT] Remote Capabilities Complete (status=0x%02X)\n",
			ev->status);
	...
	if (ev->status == 0) {
		...
		// Error - transition to stopped
		if (sm->utils)
			DBG(sm->utils,"[ERROR] Remote capabilities failed with\
				status 0x%02X\n", ev->status);
		cs_set_state(sm, CS_STOPPED);
	}
}
		...
		void *user_data)
{
	...
	if (!sm || !data ||
		size < sizeof(struct hci_evt_le_cs_config_complete)) {
		return;
	}
	...
	if (sm->utils)
		DBG(sm->utils,"[EVENT] Configuration Complete (size=0x%02X)\n",
			size);

	/* State Check */
	if(CS_WAIT_CONFIG_CMPLT != cs_get_current_state(sm)) {
		if (sm->utils)
			DBG(sm->utils,"Event received in Wrong State!!\
				Expected : CS_WAIT_CONFIG_CMPLT");
		return;
	}
	...
	if (event->status == 0) {
		...
		// Reflector role
		if (sm->utils)
			DBG(sm->utils,"Waiting for security enable event...\n");
		//ToDO : Initiator role - Send CS Security enable cmd
	} else {
		// Error - transition to stopped
		if (sm->utils)
			DBG(sm->utils,"[ERROR] Configuration failed with status 0x%02X\n",
				event->status);
		cs_set_state(sm, CS_STOPPED);
	}
	...
	if (sm->utils)
		DBG(sm->utils,"***rap_cs_config_cmplt_evt done***");
}
		...
		void *user_data)
{
	...
	if (!sm || !data ||
		size < sizeof(struct hci_evt_le_cs_security_enable_complete)) {
		return;
	}
	...
	if (sm->utils)
		DBG(sm->utils,"[EVENT] Security Enable Complete (size=0x%02X)\n",
			size);
	...
	if (event->status == 0) {
		...
		//Reflector role
		if (sm->utils)
			DBG(sm->utils,"Waiting for CS Proc complete event...\n");
		//ToDO:Initiator role - Send CS Proc Set Parameter and CS Proc enable cmd
	} else {
		// Error - transition to stopped
		if (sm->utils)
			DBG(sm->utils,"[ERROR] Security enable failed with status 0x%02X\n",
				event->status);
		cs_set_state(sm, CS_STOPPED);
	}
		...
		void *user_data)
{
	...
	if (!sm || !data ||
		size < sizeof(struct hci_evt_le_cs_procedure_enable_complete)) {
		return;
	}
	...
	if (sm->utils)
		DBG(sm->utils,"[EVENT] Procedure Enable Complete (size=0x%02X)\n", size);

	/* State Check */
	if(CS_WAIT_PROC_CMPLT != cs_get_current_state(sm)) {
		if (sm->utils)
			DBG(sm->utils,"Event received in Wrong State!!\
				Expected : CS_WAIT_PROC_CMPLT");
		return;
	}
	...
	if (event->status == 0) {
		...
		// Error - transition to stopped
		if (sm->utils)
			DBG(sm->utils,"[ERROR] Procedure enable failed with\
				status 0x%02X\n", event->status);
		cs_set_state(sm, CS_STOPPED);
		sm->procedure_active = false;
	}
...
static void parse_i_q_sample(const uint8_t **buf, int16_t *i_sample,
		int16_t *q_sample)
{
	const uint8_t *p = *buf;
	uint32_t buffer = (uint32_t)p[0] |
		...
		void *user_data)
{
	...
	if (!sm || !data || size < sizeof(struct hci_evt_le_cs_subevent_result)) {
		return;
	}
	...
	if(sm->procedure_active == false) {
		if (sm->utils)
			DBG(sm->utils,"Received CS Subevent Result event when\
				CS Procedure is inactive!!\n Discarding the event");
		return;
	}

	const uint8_t cs_role = cs_opt.role;
	const uint8_t cs_rtt_type = cs_opt.rtt_type;
	const uint8_t max_paths = min_size((event->num_ant_paths + 1),
								CS_MAX_ANT_PATHS);
	...
	if (!ev) {
		if (sm->utils)
			DBG(sm->utils,"[ERROR] Failed to allocate memory for\
				subevent result\n");
		return;
	}
	if (sm->utils)
		DBG(sm->utils,"[EVENT] Subevent Result (length=%u)\n", size);
	...
	DBG(sm->utils, "event->step_data_len : %d", event->step_data_len);
	for (uint8_t i = 0; i < steps; i++) {
		struct cs_step_data *step = &ev->step_data[i];
		if (!consume(&p, &rem, 3)) {
			DBG(sm->utils, "Truncated header for step %u", i);
			break;
		}
		...
		case CS_MODE_ZERO:
			if (!consume(&p, &rem, 3)) {
				DBG(sm->utils, "Mode 0: too short (<3)");
				break;
			}
		...
		case CS_MODE_ONE:
			if (!consume(&p, &rem, 5)) {
				DBG(sm->utils, "Mode 1: too short (<5)");
				break;
			}
			DBG(sm->utils, "CS Step mode 1");
			...
			DBG(sm->utils, "CS Step mode 1", max_paths);
			if (rem >= 2) {
		...
		case CS_MODE_TWO: {
			if (!consume(&p, &rem, 1)) {
				DBG(sm->utils, "Mode 2: too short (<1)");
				break;
			}
			step->step_mode_data.mode_two_data.ant_perm_index = p[-1];
			DBG(sm->utils, "CS Step mode 2, max paths : %d", max_paths);
			for (uint8_t k = 0; k < max_paths; k++) {
				if (rem < 4) {
					DBG(sm->utils, "Mode 2: insufficient PCT for path %u (rem=%zu)",
						k, rem);
					break;
				}
				parse_i_q_sample(&p,
				...
				if (!consume(&p, &rem, 1)) {
					DBG(sm->utils, "Mode 2: missing quality indicator for path %u",
						k);
					break;
				}
				step->step_mode_data.mode_two_data.tone_quality_indicator[k] =
					p[-1];
				DBG(sm->utils, "tone_quality_indicator : %d",
					step->step_mode_data.mode_two_data.tone_quality_indicator[k]);
				DBG(sm->utils, "[i, q] : %d, %d",
					step->step_mode_data.mode_two_data.tone_pct[k].i_sample,
					step->step_mode_data.mode_two_data.tone_pct[k].q_sample);
			}
			break;
		...
		case CS_MODE_THREE: {
			...
			if (!consume(&p, &rem, 4)) {
				DBG(sm->utils, "Mode 3: mode1 too short (<4)");
				break;
			}
			DBG(sm->utils, "CS Step mode 3");
...
send_event:
	DBG(sm->utils, "MGMT CS subevent result processed: %zu bytes,\
		needed: %zu bytes", send_len, needed);
	bt_rap_hci_cs_subevent_result_callback(0, send_len, ev, sm->rap);
	free(ev);
}
		...
		void *user_data)
{
	...
	if (!sm || !data ||
		size < sizeof(struct hci_evt_le_cs_subevent_result_continue)) {
		return;
	}
	...
	if (sm->utils)
		DBG(sm->utils,"[EVENT] Subevent Result Continue (length=%u)\n", size);
	const uint8_t cs_role = cs_opt.role;
	const uint8_t cs_rtt_type = cs_opt.rtt_type;
	const uint8_t max_paths = min_size ((event->num_ant_paths + 1),
								CS_MAX_ANT_PATHS);
...
send_event:
	DBG(sm->utils, "MGMT CS subevent result cont processed: %zu bytes,\
		needed: %zu bytes", send_len, needed);
	bt_rap_hci_cs_subevent_result_cont_callback(0, send_len, ev, sm->rap);
	free(ev);
}
		...
		struct rap_utils *utils)
{
	...
	sm = (struct cs_state_machine_t*) malloc(sizeof(struct cs_state_machine_t));
	if (!sm) { 
		if (utils)
			DBG(utils,"[ERROR] Failed to allocate state machine\n");
		return ;
	} 
	...
	if (sm->utils)
		DBG(sm->utils,"bt_hci_register done, event_id : %d", event_id);

	if (!event_id) {
		if (sm->utils)
			DBG(sm->utils,"Error: Failed to register hci le meta events\
				(event_id=0x%02X)\n", event_id);
		free(sm);
		return;
	}
