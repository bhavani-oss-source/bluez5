...
static void dbg_hexdump_line(struct rap_utils *rap, const char *prefix, const uint8_t *buf, size_t len, size_t max);
static void dbg_decode_ranging_header(struct rap_utils *rap, const uint8_t *buf, size_t len);
static void dbg_decode_ras_subevent_header(struct rap_utils *rap, const uint8_t *buf, size_t len);
#endif
...
static void dbg_hexdump_line(struct rap_utils *utils, const char *prefix, const uint8_t *buf, size_t len, size_t max)
{
	if (!buf || len == 0) {
		RAP_DBG(utils,"%s: <empty>", prefix ? prefix : "hexdump");
		return;
	}
